...
sub parselinedef($) {
    ...
    return { from => $from, to => $to, flags => $flags, type => $type,
        tag => $tag, rside => $rside, lside => $lside };
}
...
sub parsenode($) {
    ...
    ($x,$y,$dx,$dy,@lbbox[0..3],@rbbox[0..3],$left,$right)
        = unpack("ssssssssssssss",shift);
    return {
...
sub new {
    ...
    $level{rootnode}
        = treenodes(readobjects($level{map},"NODES",28, \&parsenode ));
